  ...
  _createClass(_default, [{
    ...
    value: function print(PAWindow) {
      ...
      var _loaded = function _loaded() {
        ...
        try {
          var box = document.getElementById(_this.settings.id);

          var canvasList = _this.elsdom.querySelectorAll('.canvasImg');

          // console.log(66, _this.elsdom);
    ...
    value: function write(PADocument, $ele) {
      PADocument.open();
      PADocument.write("".concat(this.docType(), "<html style='height: 600px'>").concat(this.getHead()).concat(this.getBody(), "</html>"));
      PADocument.close();
    }
  }, {
    key: "docType",
      ...
      if (closeBtn) {
        ...
        new printarea_default({
          ...
          endCallback: function endCallback() {
            // 调用打印之后的回调事件
            closeBtn = true;
            binding.value.endCallback && binding.value.endCallback(binding)
          }
        });
      }
    };
